...
from sklearn.utils import check_array, _safe_indexing, _safe_assign
from sklearn.utils import _determine_key_type, _get_column_indices, Bunch
...
def nearby(X, X_eval, feature, value, preds, threshold=0.5, type="L2", sim_type=None):
    column = _safe_indexing(X, feature, axis=1)
    positions = np.where(column==value)
    original = []
    for i in positions[0]:
        original.append(_safe_indexing(X,i,axis=0))
    original = np.array(original)
    #original contains original data points

    mask = np.zeros(X.shape[0], dtype=bool)
    for i in range(original.shape[0]): #for all original points
        result = np.tile(original[i], (X.shape[0],1))

        if (sim_type == "L2"):
            distance = np.linalg.norm(result - X_eval, axis=1)
        elif (sim_type == "L1"):
            distance = np.abs(result - X_eval).sum(axis=1)
        else:
            cosine_sim = cosine_similarity(result, X_eval).diagonal()
            distance = np.ones(cosine_sim.shape[0]) - cosine_sim

        distance = (distance - distance.min()) / (distance.max() - distance.min())
        mask = mask | (distance <= threshold)
...
def _partial_dependence_brute(est, grid, features, X, response_method,threshold,sim_type):
    predictions = []
    ...
    for new_values in grid:
        ...
        try:
            ...
            ######
            pred = nearby(X=X, X_eval=X_eval, feature=features, value=new_values, preds=pred, threshold=threshold, sim_type=sim_type)
            ######
            predictions.append(pred)
            # average over samples
            averaged_predictions.append(np.nanmean(pred, axis=0))
    ...
    *,
    ...
    if method == "brute":
        averaged_predictions, predictions = _partial_dependence_brute(
            estimator, grid, features_indices, X, response_method,threshold,sim_type
        )

        #predictions = predictions[~np.isnan(predictions).any(axis=1)]
        predictions = np.nan_to_num(predictions)
        # reshape predictions to
        # (n_outputs, n_instances, n_values_feature_0, n_values_feature_1, ...)
        predictions = predictions.reshape(
            -1, predictions.shape[0], *[val.shape[0] for val in values]
        )
